...
"""
def adTranspose(v, w, L, K, Gx, Gy, Gz):
    ...
    L, K, Gx, Gy, Gz = map(to_tensor, [L, K, Gx, Gy, Gz])
    ...
    m[..., 2] = torch.fft.ifftn(torch.fft.fftn(w[..., 2]) * L).real
    ...
    adTvw[..., 2] = torch.fft.ifftn(vft * Gz).real * m[..., 0]

    vft = torch.fft.fftn(v[..., 1])
    adTvw[..., 0] += torch.fft.ifftn(vft * Gx).real * m[..., 1]
    adTvw[..., 1] += torch.fft.ifftn(vft * Gy).real * m[..., 1]
    adTvw[..., 2] += torch.fft.ifftn(vft * Gz).real * m[..., 1]

    vft = torch.fft.fftn(v[..., 2])
    adTvw[..., 0] += torch.fft.ifftn(vft * Gx).real * m[..., 2]
    adTvw[..., 1] += torch.fft.ifftn(vft * Gy).real * m[..., 2]
    adTvw[..., 2] += torch.fft.ifftn(vft * Gz).real * m[..., 2]
    ...
    pft = torch.fft.fftn(m[..., 0] * v[..., 2])
    adTvw[..., 0] += torch.fft.ifftn(pft * Gz).real
    ...
    pft = torch.fft.fftn(m[..., 1] * v[..., 2])
    adTvw[..., 1] += torch.fft.ifftn(pft * Gz).real

    # --

    pft = torch.fft.fftn(m[..., 2] * v[..., 0])
    adTvw[..., 2] += torch.fft.ifftn(pft * Gx).real

    pft = torch.fft.fftn(m[..., 2] * v[..., 1])
    adTvw[..., 2] += torch.fft.ifftn(pft * Gy).real

    pft = torch.fft.fftn(m[..., 2] * v[..., 2])
    adTvw[..., 2] += torch.fft.ifftn(pft * Gz).real
    ...
    adTvw[..., 2] = torch.fft.ifftn(K * torch.fft.fftn(adTvw[..., 2])).real
