...
function [dscMean, Jacs, tNegJacs] = getAllMetricsNIREP(phi, patient)
    ...
    moving_f = sprintf('%s/OASIS_%04d_0000.nii', data_dir, patient);
    % disp(moving_f);
    system(sprintf('gunzip %s.gz', moving_f));
    moving_nii = load_nii(moving_f);
    moving = moving_nii.img;
    system(sprintf('gzip %s', moving_f));

    fixed_f = sprintf('%s/OASIS_%04d_0000.nii', data_dir, patient + 1);
    system(sprintf('gunzip %s.gz', fixed_f));
    fixed_nii = load_nii(fixed_f);
    fixed = fixed_nii.img;
    system(sprintf('gzip %s', fixed_f));

    fprintf('IMAGENES CARGADAS')


    data_dir = '/home/epdiff/python/3D/Data/OASIS_seg';

    % Load the segmentations
    moving_f = sprintf('%s/OASIS_%04d_0000.nii', data_dir, patient);
    system(sprintf('gunzip %s.gz', moving_f));
    moving_nii = load_nii(moving_f);
    movingSegmentation = moving_nii.img;
    system(sprintf('gzip %s', moving_f));

    fixed_f = sprintf('%s/OASIS_%04d_0000.nii', data_dir, patient + 1);
    system(sprintf('gunzip %s.gz', fixed_f));
    fixed_nii = load_nii(fixed_f);
    fixedSegmentation = fixed_nii.img;
    system(sprintf('gzip %s', fixed_f));
