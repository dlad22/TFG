...
@ mhg
Para imagenes 3D habra que llamarlo 4D.
"""
...
class FNO3d(nn.Module):
    ...
    def get_grid(self, shape, device):
        ...
        # batchsize, size_x, size_y, dim, size_z = shape[0], shape[1], shape[2], shape[3], shape[4]
        batchsize, size_x, size_y, size_z, dim = shape[0], shape[1], shape[2], shape[3], shape[4]
...
if __name__ == "__main__":
    ...
    class DatasetArray(torch.utils.data.Dataset):
        ...
        def __getitem__(self, index):
            ...
            X = self.v0[index]
            XX = self.v[index]
            # X = torch.tensor(X) # Comentado por Alvaro
            # XX = torch.tensor(XX)
            ...
            X = X.reshape(X.shape[0], X.shape[1], 1, 2).repeat([1,1,T,1])
            # X = X.permute(0, 1, 3, 2)

            # X = X.reshape(192,192,1,2).repeat([1,1,T,1])
    ...
    # y_normalizer.cuda() # mhg - Cuidado, utilizan una normalizacion Gaussiana. Si no aprende podria ser por esto...
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            # out = out.view(batch_size, 96, 96, 2, T)
            ...
            # loss = myloss(out.reshape(batch_size, -1), y.view(batch_size, -1))
        ...
        if ep > epochs - 50:
            ...
            """
            print( "...Como se que va a petar aqui..." )
            if use_encoding:
                torch.save(model, 'BullEye_FNO3D_GaussNorm_model.pt')
            else:
                torch.save(model, 'BullEye_FNO3D_model.pt')
            """
            ...
            with (torch.no_grad()):
                ...
                for x, y in val_loader:
                    ...
                    out = model(x)
                    out = out.permute(0, 1, 2, 4, 3)
                    # out = out.view(out.shape[0], 96, 96, 2, T)
                    # out = y_normalizer.decode(out)
                    # val_loss = myloss(out.view(out.shape[0], -1), y.view(out.shape[0], -1))
    ...
    if use_encoding:
        torch.save(model, 'BullEye_FNO3D_T_GaussNorm_model.pt')
    else:
        torch.save(model, 'BullEye_FNO3D_T_model.pt' )
    ...
    #             scipy.io.savemat('BullEye_FNO3D_%s' % test_EPDiffv_f[j][16:], { 'v': np.squeeze(vv[i, :, :, :, :].numpy())})
    #             j = j + 1
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            # Bug https://github.com/neuraloperator/neuraloperator/issues/57
            # out = model(x)
